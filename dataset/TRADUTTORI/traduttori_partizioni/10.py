...
def verifica_colonne(df):
    ...
    # Verifica colonne essenziali (dopo la rinomina)
...
def traduci_testo_sicuro(testo, lingua_originale, max_retries=2):
    """
    ...
    # Limita lunghezza per evitare errori API
    ...
    for tentativo in range(max_retries):
        try:
            ...
            if "Connection" in error_type or "Remote" in error_type:
                print(f"   --> Errore connessione, tentativo {tentativo + 1}/{max_retries}")
                time.sleep(5)
...
def traduci_partizione(file_path_partizione):
    ...
    for idx, row in tqdm(df.iterrows(), total=len(df), desc="Traduzione"):
        ...
        try:
            ...
            if lingua == 'en':
                ...
                titolo_tradotto = traduci_testo_sicuro(row['post_title'], lingua)
    ...
    file_output = os.path.join("TRADUTTORI/subset_tradotti", nome_file)  # NUOVO PERCORSO
...
if __name__ == "__main__":
    ...
    partizione_path = "TRADUTTORI/subset/partizione_10_di_10.csv"
